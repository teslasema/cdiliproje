#include <stdio.h>
#include <stdlib.h>
//alýsveris uygulamasi yaptým giyim alýmýnda yan urunler indirimli 
//eger sali günü ise yan urunler hediye edilecek.gun icinde max 100 musteri.
//musteri sayýsýný döngü icinde kullandým.uygun beden secimi icin fonksiyona dahil edip beden hesaplamasýný yaptýrdým.
//renk secimi icin switch case kullandým.
//gun sonunda toplam musteri sayýsý,toplam kazanci yazdýran projemde kullanýcý verilerini dosyaya yazdýrmasýný saðladým.
// dosyalama kullanarak.dizileri katagoriler kýsmýnda kullandým.
//karakter dizini karsilama metninde kullandým.
...
int main() {
    ...
    float boy, kilo, VKI;
    int kullanici_adi;
    int max_musteri = 100; // Maksimum müþteri sayýsý
    int musteri_sayisi = 0; // Þu anki müþteri sayýsý
    ...
    //karsilama metni
    char karsilama_metni[100]="MAGZAMIZA HOSGELDÝNÝZ";
    ...
    const char* ust_giyim[] = {"Bluz", "Kazak", "Tiþört", "Gömlek"};
    const char* alt_giyim[] = {"Etek", "Pantolon", "Þort"};
    ...
    while (musteri_sayisi < max_musteri) {
        ...
        printf("Hoþgeldiniz %d\n", kullanici_adi);

        // Kategori seçimi
        printf("Almak istediðiniz ürünün kategorisini seçiniz:\n");
        ...
        if (katagori == 1) {
            ...
            // Saat promosyonu
            printf("Ýndirimli saat almak ister misiniz? (1: Evet, 0: Hayýr): ");
            scanf("%d", &kemer);
            if (kemer == 1) {
                printf("Saat aldini! Yüzde 50 indirim uygulanacak.\n");
                toplam_kazanc += 10; // Ýndirimli fiyat
            ...
            // Kemer promosyonu
            printf("Ýndirimli kemer almak ister misiniz? (1: Evet, 0: Hayýr): ");
            scanf("%d", &kemer);
            if (kemer == 1) {
    ...
    if (kemer==1 || saat==1) { 
        // Saat veya kemer promosyon seçimi 
            ...
            scanf("%d", &gun); 
            if (gun == 2) { // 2 Salýyý temsil ediyor 
...
switch(renk) {
    ...
    default:
        ...
        if (VKI >= 15 && VKI < 19) {
            ...
            printf("Size uygun beden mevcut deðil.\n");
        }

        // Alýþveriþe devam etme seçimi
        printf("Alisveriþe devam etmek istiyor musunuz? (1: Evet, 0: Hayýr): ");
        int devam;
        scanf("%d", &devam);
        if (devam == 0) {
            printf("Alisveriþ için teþekkür ederiz.\n");
            break;
        }
    }

    if (musteri_sayisi >= max_musteri) {
        printf("Günlük maksimum müþteri sayisina ulasildi.\n");
    }

    // Gün sonu toplam kazanç ve müþteri sayýsý
    printf("\nGün sonu raporu:\n");
    printf("Toplam müþteri sayisi: %d\n", musteri_sayisi);
